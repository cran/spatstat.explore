...
#'
#'  $Revision: 1.11 $  $Date: 2026/09/12 03:18:19 $
#'
...
transect.im <- function(X, ..., from="bottomleft", to="topright",
                        nsample=512, click=FALSE, add=FALSE, curve=NULL) {
  Xname <- short.deparse(substitute(X))
  Xname <- sensiblevarname(Xname, "X")
  stopifnot(is.im(X))
  co <- getTransectCoords(X, from=from, to=to, nsample=nsample,
                          click=click, add=add, curve=curve)
  with(co, {
    ...
    result <- fv(df,
                 ...
                 unitname = tunits,
                 fname = Xname)
    attr(result, "SamplePoints") <- ppp(x, y, window=Frame(X))
    return(result)
  })
}

transect.funxy <- function(X, ..., from="bottomleft", to="topright",
                           nsample=512, click=FALSE, add=FALSE, curve=NULL) {
  Xname <- short.deparse(substitute(X))
  Xname <- sensiblevarname(Xname, "X")
  if(!inherits(X, "funxy"))
    stop("X should be a function inheriting class 'funxy'", call.=FALSE)
  co <- getTransectCoords(X, from=from, to=to, nsample=nsample,
                          click=click, add=add, curve=curve)
  with(co, {
    ## look up function values (may be NA)
    v <- X(x, y)
    ## package into fv object
    df <- data.frame(t=t, v=v)
    colnames(df) <- c(tname, Xname)
...
getTransectCoords <- function(X, ..., from="bottomleft", to="topright",
                              nsample=512, click=FALSE, add=FALSE, curve=NULL) {
  check.1.integer(nsample)
  if(length(curve)) {
    ## parametric curve specified by user
    ...
    if(click) {
      ...
      R <- as.rectangle(X)
      from <- specifyLocationAroundFrame(from, R)
      to   <- specifyLocationAroundFrame(to,   R)
    }
  ...
  result <- list(t=t,
                 x=x, y=y,
                 tname=tname, tdescrip=tdescrip, tunits=tunits)
  return(result)
}

specifyLocationAroundFrame <- function(loc, rect) {
